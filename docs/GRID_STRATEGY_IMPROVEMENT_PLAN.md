# Crypto Options Aggregator — 网格策略优化实施方案 v2

> **文档性质**: 实施规范（给执行AI用）
> **策略核心**: 长期持有BTC + Sell Put/Call 网格做多 = 备兑轮 + 网格收租
> **编写者**: Hermes（总指挥）
> **日期**: 2026-04-12
> **当前版本**: v6.3.0（部分完成，非全部完成）
> **v2 更新**: 全面代码审计后补充验证结果、具体bug清单、竞品差距分析、收紧执行要求

---

## 第零章：现状确认

### 0.1 已验证完成项（v6.0.1-v6.0.8，不要重复做）
...
| Date.UTC 时间解析 | ✅ |
| Binance Greeks 用 API 真实值 (v6.0.3) | ✅ |
| Binance scoring 归一化 (v6.0.4) | ✅ |

### 0.2 验证清单实际完成状态（截至 v6.3.0）
...
| # | 严重度 | 位置 | 问题描述 |
|---|--------|------|---------|
| B1 | 🔴 高 | main.py:1840 | `_estimate_delta()` 标记 DEPRECATED 但仍在 Deribit fallback 中调用。删除此函数，Deribit 缺 delta 时直接用 BS 公式计算 |
| B2 | 🔴 高 | main.py (多处) | 11处 `except: pass` 静默吞异常（3处 ValueError 可接受，8处需改） |
| B3 | 🔴 高 | main.py + app.js + options_aggregator.py | STRATEGY_PRESETS 三处重复定义，改一个忘改另一个就出 bug |
| B4 | 🟡 中 | main.py + deribit_options_monitor.py | `_parse_inst_name` 重复定义（应全项目只保留一处） |
| B5 | 🟡 中 | index.html:201-202 | 风险阈值 `$55,000` 和 `$45,000` 硬编码在 HTML 中，未从 config.py 读取 |
| B6 | 🟡 中 | options_aggregator.py:305-312 | preset 在 format_report 之后才应用，presets 实际不影响扫描结果 |
| B7 | 🟢 低 | main.py | 4个 spot price 函数做同一件事，应合并为1个统一入口 |
| B8 | 🟢 低 | main.py | premium 字段名不统一（premium_usd / premium / mark_price） |

### 0.4 竞品差距分析

对比 Deribit Position Builder、Greeks.live、OptionStrat、wheelstrategy.io：

| 功能 | 本项目 | 竞品 | 优先级 |
|------|--------|------|--------|
| **可视化Payoff图** | ❌ | ✅ Deribit/OptionStrat | P0 |
| **网格多档位推荐** | ❌（只推荐1个） | ❌（竞品也不做=差异化优势） | P0 |
| **Wheel Strategy ROI** | ❌ | ✅ wheelstrategy.io | P0 |
| **情景模拟 What-If** | ❌ | ✅ OptionStrat 滑块 | P1 |
| **Put/Call IV偏度图** | 部分(DVOL) | ✅ Greeks.live | P1 |
| **风险热力图** | ❌ | ✅ Greeks.live | P1 |
| **持仓跟踪/日记** | ❌ | ✅ PremiumTracker | P2 |
| **API实时数据** | ✅ | 多数依赖外部 | ✅ 领先 |
| **DVOL波动率分析** | ✅ | ❌ 大多数竞品没有 | ✅ 独有优势 |

**结论**：本项目最大差异化优势是"网格多档位推荐"（竞品都没做）+ DVOL分析（竞品少有）。补齐 Payoff 可视化和情景模拟后就领先 90% 工具。
...
### 1.2 统一 instrument 解析（消除重复 + 修Bug B1/B4）

当前状态：
- `main.py: _parse_inst_name()` — 解析 instrument 名称
- `deribit_options_monitor.py: _parse_inst_name()` — 重复
- `main.py: _estimate_delta()` — 标记为 DEPRECATED 但仍被调用（Bug B1）

**要求**：
- 全项目只保留 `services/instrument.py` 中的 `_parse_inst_name()`
- **删除** `_estimate_delta()` 函数（Deribit 缺 delta 时改用 BS 公式直接计算，不要 fallback 到废弃函数）
- 所有文件 import from `services.instrument`
- `deribit_options_monitor.py` 也改为 import

### 1.3 统一 spot price 获取（修Bug B7）
...
- 搜索时用 `grep -n "except.*pass\|except:" dashboard/main.py` 确认全部找到
...
### 1.7 STRATEGY_PRESETS 统一（修Bug B3）

当前：`options_aggregator.py` 4次 + `main.py` 1次 + `app.js` 2次 = 3套定义

**要求**：
- 后端只在 `config.py` 定义一套
- `options_aggregator.py` 和 `main.py` 都 import from config
- `app.js` 通过 API `GET /api/config/presets` 获取（不在前端硬编码）
- 删除 app.js 中的 hardcoded STRATEGY_PRESETS 对象

### 1.8 修复 options_aggregator.py preset bug（修Bug B6）

**问题**：Preset 在 `format_report()` 之后才应用到 args，所以 presets 实际不影响扫描结果。

**要求**：把 preset 应用逻辑移到 `format_report()` 调用之前。
...
### 3.2 改造主合约表格（修Bug B5 + 用户体验）

**当前问题**：21列太多，移动端不可用。风险阈值硬编码。
...
- 添加 max-height + 内部滚动
- 风险阈值从 config.py 读取，不再硬编码

### 3.3 新增图表

1. **Payoff 可视化图** — 在价格轴上画出所有网格档位的组合盈亏曲线（对标 OptionStrat/Deribit Position Builder）
2. **网格档位分布图** — 在价格轴上标注所有档位的行权价，用颜色区分 Put/Call
...
| 阶段 | 内容 | 预估工作量 | 前置条件 |
|------|------|-----------|---------|
| **Phase 1** | 1.1 拆分 main.py（步骤1-9）+ 修 Bug B1/B4 | 大 | 无 |
| **Phase 2** | 1.3-1.8 架构清理 + 修 Bug B2/B3/B5/B6/B7/B8 | 中 | Phase 1 |
| **Phase 3** | 2.2 网格引擎 + 2.2.3 API 端点 | 大 | Phase 2 |
| **Phase 4** | 3.1-3.3 前端网格策略Tab + Payoff图 | 大 | Phase 3 |
| **Phase 5** | 3.4 通知 + 4.x 代码质量 | 小 | Phase 4 |
...
- [ ] 没有循环 import
- [ ] `_estimate_delta` 已删除
- [ ] `_parse_inst_name` 全项目只有一处定义（在 services/instrument.py）

### Phase 2 验证
- [ ] `get_spot_price()` 统一入口工作正常
- [ ] 全项目 0 处 broad `except: pass`（ValueError 除外）
- [ ] SQLite WAL 模式启用
- [ ] STRATEGY_PRESETS 全项目只在 config.py 定义一处
- [ ] `GET /api/config/presets` 返回正确数据
- [ ] 风险阈值不再硬编码在 HTML 中
- [ ] premium 字段名统一为 premium_usd

### Phase 3 验证
- [ ] `GET /api/grid/recommend` 返回正确格式的网格推荐
- [ ] 评分公式各权重正确实现
- [ ] 波动率方向信号逻辑正确
- [ ] 情景模拟计算准确（与手动计算对比）
- [ ] 空数据时优雅降级（返回空列表而非 500）

### Phase 4 验证
- [ ] 网格策略 Tab 正确显示
- [ ] Put/Call 表格数据与 API 一致
- [ ] Payoff 图正确显示组合盈亏曲线
- [ ] 风险热力图颜色编码正确
- [ ] 情景模拟滑块交互流畅
- [ ] 移动端基本可用（不求完美）
...
1. **必须按 Phase 顺序执行**，不能跳步。Phase 1 没做完不许碰 Phase 3
2. **每个 Phase 完成后必须跑验证清单**，全部打勾才能进下一 Phase
3. **不能标 ✅ 除非代码实际已改**。Hermes 会逐项验证 diff
4. **每完成一个 Phase，commit 并 push**，commit message 格式：`feat(phase N): 简短描述`
5. **main.py 拆分后必须 ≤ 300 行**，否则 Phase 1 不算完成
6. **Bug B1-B8 必须在 Phase 1-2 中全部修复**，不能留到后面
7. 如果某步不确定，先问再做，不要猜
