...
# Fix Windows Unicode display issues
...
class DeribitOptionsMonitor:
    ...
    def get_options_recommendations(
        ...
        # 新增保证金率参数
        margin_ratio: float = 0.2, # 20% Initial Margin Ratio
        option_type: str = "PUT",
    ) -> dict[str, Any]:
        """获取期权推荐 (支持 PUT 或 CALL)，带流动性过滤。"""
        currency = self._normalize_currency(currency)
        ...
        s_min, s_max = -1.0, -1.0
        if strike_range:
            parts = strike_range.split("-")
            if len(parts) == 2:
                s_min, s_max = float(parts[0]), float(parts[1])

        target_suffix = "-P" if option_type.upper() == "PUT" else "-C"

        # 第一轮筛选：基本条件
        for summary in summaries:
            instrument_name = summary.get("instrument_name", "")
            if not instrument_name.endswith(target_suffix):
                continue
            ...
            # 行权价过滤
            if strike is not None and meta.strike != float(strike):
                continue
            if strike_range and not (s_min <= meta.strike <= s_max):
                continue
        ...
        for item in candidates:
            ...
            # 基于保证金占用的 APR (SPAN 保证金约为行权价的 15-30%)
            margin_required = item["strike"] * margin_ratio
            apr = premium_usd / margin_required * (365 / item["dte"]) * 100
            
            # 传统 APR 参考
            trad_apr = premium_usd / item["strike"] * (365 / item["dte"]) * 100
            ...
            pick = {
                ...
                "dte": item["dte"],
                "delta": round(delta, 4),
                "gamma": round(float(greeks.get("gamma") or 0.0), 6),
                "vega": round(float(greeks.get("vega") or 0.0), 2),
                "mark_iv": round(mark_iv, 2),
    ...
    def run_scan(
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            ...
            future_sell = executor.submit(
                self.get_options_recommendations,
                currency,
                max_delta,
                min_apr,
                min_dte,
                ...
                option_type="PUT" # 保持默认扫描为PUT，如果后续需要可以通过外部传参
            )
